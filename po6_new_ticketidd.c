...
typedef struct cancl{
    dateData date;
    char flightNo[100],userId[100],ticketId[100];
}cancel;
...
void reservation(query Query){
    ...
    strcat(data.ticketId,data.userId);
    strcat(data.ticketId,data.flightNo);
    strcat(data.ticketId,data.source);
...
bool ticketCancel(loginCred data){
    ...
    FILE *outfile;
    outfile=fopen("cancelList1.txt","a");
    if(outfile==NULL){
        outfile=fopen("cancelList1.txt","w");
    }
    cancel Cancel;
    strcpy(Cancel.userId,data.id);
    char tmp[10];
    scanf("%s",Cancel.ticketId);
...
void ticketStatus(loginCred data){
    ...
    infile3=fopen("cancelList1.txt","r");
    ...
    if(tmp==1||tmp==2){
        ...
        if(flag2==true&&tmp==1){
            printf("1).Enter the ticket Id for seeing the status : ");
            char ticketid[100];
            scanf("%s",ticketid);

            while(fread (&ticket, sizeof(reserve), 1, infile)){
                    //printf("id : %s \n,",ticket.userId);        //debugging
            ...
            if(infile3!=NULL)
                while((fread (&Cancel, sizeof(cancel), 1, infile3))){
                    printf("flighr %s  id %s \n",Cancel.ticketId,Cancel.userId);//amanpurwar    //debugging
                    if(strcmp(ticketid,Cancel.ticketId)==0){
                        flag1=false;
                    }
        ...
        if(tmp==3){
            ...
            if(flag2==true){
                    ...
                    while(fread (&ticket, sizeof(reserve), 1, infile)){
                        ...
                        if(strcmp(ticket.userId,data.id)==0){
                            printf("\tTicketID : %s\tPass. name : %s %s Date: %d:%d:%d\n\tFlight No. : %s\tSource : %s\tDest. : %s\n"
                        ,ticket.ticketId,ticket.passfname,ticket.passlname,ticket.date.dd,ticket.date.mm,ticket.date.yyyy,ticket.flightNo,ticket.source,ticket.dest);
                        }
                    }
            }
            else{
